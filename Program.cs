...
namespace Xtensive.Orm.Migration
{
  internal class Program
  {
    ...
    private static void ShowDetail(ulong total, ulong processed, long tiks)
    {
      if (processed > 10)
      {
        var totalTiks = (long)(((float)tiks) / processed * total);
        var expTimeSpan = TimeSpan.FromTicks(totalTiks-tiks);
        Console.WriteLine($"Dur {TimeSpan.FromTicks(tiks):hh\\:mm\\:ss}   Exp: {expTimeSpan:hh\\:mm\\:ss}    EndAt: {DateTime.Now + expTimeSpan}");
      } else
      {
        Console.WriteLine($"Dur {TimeSpan.FromTicks(tiks):hh\\:mm\\:ss}");
      }
      var persent = ((double)processed / total) * 100; ;
      Console.Write($"{processed:N0} / {total:N0}    {persent:N0}%");
    }
    ...
    static void Main(string[] args)
    {
      ...
      ulong totalRecorsExpected = 0;
      ulong totalRecorsMigrated = 0;
      var totalRecordsExpectedDict = new Dictionary<int, ulong>();
      InLoggerAction("counting ", () =>
      {
        ...
        for (var i = 0; i < tablesCount; i++)
        {
          ...
          totalRecorsExpected += src.InReadTransaction(srcConnection =>
          {
            var srcSqlTable = SqlDml.TableRef(mapTable.SrcTable);
            var countQuery = SqlDml.Select(srcSqlTable);
            countQuery.Columns.Add(SqlDml.Count());
            using (var countCmd = srcConnection.CreateCommand(countQuery))
            {
              var result = (ulong)Convert.ChangeType(countCmd.ExecuteScalar(), typeof(ulong));
              totalRecordsExpectedDict.Add(i, result);
              return result;
            }
          });
        }
      ...
      for (var i = 0; i < tablesCount; i++)
      {
        ...
        var consoleTableProgresPosiotion = Console.GetCursorPosition();
        ClearLine();
        Console.WriteLine();
        ShowDetail(totalRecorsExpected, totalRecorsMigrated, timer.ElapsedTicks);
        var consoleTotalProgresPosiotion = Console.GetCursorPosition();
        consoleTableProgresPosiotion.Top = consoleTotalProgresPosiotion.Top - 2;
        consoleTotalProgresPosiotion.Top--;
        consoleTotalProgresPosiotion.Left = 0;
        ...
        src.InReadTransaction(srcConnection =>
        {
          dst.DoAction(dstConnection =>
          {
            ...
            var countStr = $" / {totalRecordsExpectedDict[i]:N0}";
            ...
            using (var selectCmd = srcConnection.CreateCommand(selectQuery))
            {
              ...
              while (reader.Read())
              {
                ...
                totalWrited++;
                totalRecorsMigrated++;
                if (itemsInTransaction >= 50)
                {
                  dstConnection.Commit();
                  dstConnection.BeginTransaction();
                  itemsInTransaction = 0;
                }
                if ((totalRecorsMigrated & 0xff) == 0)
                {
                  Console.SetCursorPosition(consoleTableProgresPosiotion.Left, consoleTableProgresPosiotion.Top);
                  Console.Write($"{totalWrited:N0}{countStr}");
                }
                if ((totalRecorsMigrated & 1023) == 0)
                {
                  Console.SetCursorPosition(consoleTotalProgresPosiotion.Left, consoleTotalProgresPosiotion.Top);
                  ShowDetail(totalRecorsExpected, totalRecorsMigrated, timer.ElapsedTicks);
                }
              }
